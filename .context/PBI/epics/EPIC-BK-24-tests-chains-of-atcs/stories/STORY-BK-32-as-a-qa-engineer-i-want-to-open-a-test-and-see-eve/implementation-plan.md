...
> Jira field: `customfield_10165` · [View in Jira](https://jira.upexgalaxy.com/browse/BK-32)

# [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) — Implementation Plan (Dev)

> Story: ***TMS-Test View | View a test with all chained ATCs expanded*** — 3 SP, epic [https://jira.upexgalaxy.com/browse/BK-24#icft=BK-24](https://jira.upexgalaxy.com/browse/BK-24#icft=BK-24) (Tests — chains of ATCs). Formal dependency: [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) (shipped: `tests`/`test*steps` schema + `bunkai*create_test`).
Glossary canon applies throughout: ***Test**** (capital T) = named container owning an ordered ****chain**** of ****ATC**** references — references, never copies. Canonical join table is ****test_steps***. Banned: "test case" for a chain, "Atomic Test Component", "published ATC", "snapshot" for this view (live content only).
...
- A routed server-component page `/projects/{projectSlug}/tests/{testId`} reached by clicking an explorer Tests-group row ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) wired the rows as non-navigating divs; [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) makes them navigate).

Both consume ***one server-side read rulebook*** — a SECURITY DEFINER RPC `bunkai*get*test*expanded(p*actor*user*id, p*test*id)` returning composed jsonb (header + ordered chain; each chain item carries the live ATC header + ordered steps + ordered assertions). Strictly READ-ONLY: no drag handles, no edit/add/remove/reorder controls. Designed to be cleanly extensible for [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) (reorder), but [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) builds only the read path.
...
- **Reuse** **`bunkai**__***assert***_***actor***_***can***_***write***__`* — rejected: denies viewers (violates §3.4).
...
- **Parse** `expand` **and conditionally compose** — rejected for MVP (see Decision 4): always return fully expanded; the `expand` param is accepted and ignored (documented), because the only consumer needs the full expansion and conditional composition is dead weight that [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) does not need either.
...
- ✅ Extensible for [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28): the chain item already carries `position`; [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) layers a PATCH reorder RPC + drag UI on top without changing this read shape.
...
step*id,                             – test*steps.id (chain-row id; STABLE handle [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) reorder needs)
...
Real columns verified: `atc*steps(id, atc*id, position, content, input*data, expected)`, `atc*assertions(id, atc*id, position, content)` (`0004*atcs.sql:179-291`), both ordered by `position`. The outer `atcs` array is ordered by `test*steps.position` via `jsonb*agg(... order by ts.position)`. Live content: joins `atcs`/`atc*steps`/`atc*assertions` at query time. Includes `step*id` (the `test*steps.id`) so [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) has a stable per-row handle to reorder; harmless for [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) (used only as a React key).
...
- `lib/tests/errors.ts` ***(modify)*** — extend the existing `mapTestRpcError` ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27)) to handle the read path: `P0002` → `ApiError('not*found', 'Test not found.', { details: { reason: 'not*found' } })`; default → `internal_error`. (`42501` mapping already exists from BK-27; the read RPC never raises it, but keeping the case is harmless.)
- `lib/tests/errors.test.ts` ***(modify)*** — add a `P0002 → not_found` row.
- ***Error envelope / OpenAPI registry — NO change.*** Reuse the canonical `not*found` code (already in `API*ERROR*CODES` and the registry enum). We deliberately do NOT add a `test*not*found` code: a distinct code would itself be a weak existence signal, and `not*found` is the established non-disclosure code (mirrors [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) Decision 5 and `mapAtcRpcError` P0002). No `error-envelope.ts` / `registry.ts` edits needed for [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32).
...
- `auth: 'required'` with NO `requires: ['atc:write']` — read uses the session/PAT identity only; viewer-role read is gated by the RPC's read-level membership check, not a write capability (§3.4, [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) Decision 16 parity for the read direction).
...
- ATCDetail "Used by" chain-row anatomy — `8px 12px`, `bg-2`, `1px stroke-2`, `--r-2`, branch icon `12px fg-3`, mono id `11px fg-3`, layer chip (`project.jsx:528-546`); reused by [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) for chain rows and reused here for each ATC card header.
...
> Note `projectSlug` here is the route's display context only; Tests are workspace-scoped ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) D9), so the read is keyed by `testId` + active-workspace membership, not by project.
...
***Task:*** Wrap each `explorer-test-{id`} row in a `Link` to `/projects/{projectSlug}/tests/{id`} (mirror `AtcTable`), preserving markup + testid; keep [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) accordion/collapse/resize behaviors intact.
