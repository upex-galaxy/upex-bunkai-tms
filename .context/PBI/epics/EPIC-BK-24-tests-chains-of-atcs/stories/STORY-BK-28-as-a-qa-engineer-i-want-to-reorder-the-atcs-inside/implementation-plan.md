...
> Jira field: `customfield_10165` · [View in Jira](https://jira.upexgalaxy.com/browse/BK-28)

## Goal

Let a workspace member reorder the ATC chain inside an existing Test, preserving the exact multiset of chained ATCs. One server-side rulebook (`bunkai*reorder*test_steps` SECURITY DEFINER RPC) consumed identically by the headless `PATCH /api/v1/tests/{id}/reorder` surface and the UI drag-reorder on the [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) Test detail page. Optimistic locking, no-op detection, atomic position rewrite, single `test.reordered` activity-log event.

## Key design decisions

- ***Reorder handle =**** `step*id` ****(test*steps.id), NOT atc*id.**** The 0025 read RPC already declares `step*id` as "the stable per-row chain handle ... the reorder handle [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) will need" (0025:69). A chain may legally hold the same `atc*id` at multiple positions (surrogate PK, no `unique(test*id, atc*id)` — BR-5), so atc*id cannot identify a row. The request body is an ordered array of the test's existing `step*id}}s, permuted. This cleanly resolves the AC narrative (written in atc*id terms) against the duplicate-ATC business rule. The activity-log {{old*chain`/`new*chain` stay as arrays of ****atc_id*** (human-meaningful run order). Flag for PM/glossary in review.
- ***Optimistic locking via**** `version` ****column +**** `X-If-Match` ****custom header***, mirroring the [https://jira.upexgalaxy.com/browse/BK-96#icft=BK-96](https://jira.upexgalaxy.com/browse/BK-96#icft=BK-96) ATC precedent. The `tests` table has no `version` today ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) omitted it) — [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) adds `version int not null default 1` (the architect clarification said [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28) owns this migration if [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) skips it). Reuse `readVersionPrecondition` (lib/atcs/optimistic-lock.ts) verbatim. Lenient mode: absent header skips the check; version still bumps on real change.
...
- ***Dedicated sub-resource*** `PATCH /api/v1/tests/{id}/reorder` (BR: body is the complete new order, not a diff). Capability gate `atc:write` (same as Test create — the system has no separate `test:write` capability; [https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) used `atc:write`).
- ***UI is a ratified DERIVATION*** extending the [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) read-only detail page (design plan §8 D9/D10 — `TestDetail` mockup is a placeholder; reorder UX explicitly deferred to [https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28](https://jira.upexgalaxy.com/browse/BK-28#icft=BK-28)). Frozen §2 tokens only. Add `@dnd-kit` (core + sortable + utilities). Viewer role: drag handles absent, no Save.
...
### Step 5 — UI drag-reorder on the [https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32](https://jira.upexgalaxy.com/browse/BK-32#icft=BK-32) detail page
...
- Error codes 45123/45124/45125 follow the tests-domain 451xx block ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27) used 45120-45122).

## Out of scope (per story)

Add/remove ATCs ([https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27](https://jira.upexgalaxy.com/browse/BK-27#icft=BK-27)), Test metadata edit, runs/exec order, activity-log VIEWING UI, undo/rollback, bulk reorder, diff visualization, live teammate notification.
