...
> Jira field: `customfield_10165` · [View in Jira](https://jira.upexgalaxy.com/browse/BK-18)

# [https://jira.upexgalaxy.com/browse/BK-18#icft=BK-18](https://jira.upexgalaxy.com/browse/BK-18#icft=BK-18) — Plan de Implementación (Dev)
...
API REST transaccional para ***crear y editar ATCs**** (Acceptance Test Cases) con sus `steps` y `assertions` en una sola llamada. Cabeza de la cadena ATC — [https://jira.upexgalaxy.com/browse/BK-19#icft=BK-19](https://jira.upexgalaxy.com/browse/BK-19#icft=BK-19)/20/21/22/23/27 dependen de este contrato. Consumidor primario = CLI/scripts vía ****PAT (bearer)***; la UI ([https://jira.upexgalaxy.com/browse/BK-19#icft=BK-19](https://jira.upexgalaxy.com/browse/BK-19#icft=BK-19)) aún no existe.

## Hallazgo de diseño bloqueante (y su resolución)

El RPC existente `bunkai*save*atc` (0007) es `SECURITY INVOKER` y depende de `auth.uid()` vía RLS. [https://jira.upexgalaxy.com/browse/BK-18#icft=BK-18](https://jira.upexgalaxy.com/browse/BK-18#icft=BK-18) autentica con ***PAT bearer*** → no hay JWT → `auth.uid()` es `NULL`. Los helpers `bunkai*can*write_workspace(ws)` también leen `auth.uid()` internamente, así que el patrón DEFINER de los RPCs de módulos (que asume sesión cookie) tampoco aplica directo.
...
- ***Prueba en vivo (evidencia, no commiteada)***: Supabase MCP `execute*sql` — seed → `bunkai*create*atc` happy + `ac*outside*user*story` (assert rollback: count hijos = 0) + `module*outside*project_subtree` → cleanup. Igual precedente [https://jira.upexgalaxy.com/browse/BK-15#icft=BK-15](https://jira.upexgalaxy.com/browse/BK-15#icft=BK-15)/[https://jira.upexgalaxy.com/browse/BK-17#icft=BK-17](https://jira.upexgalaxy.com/browse/BK-17#icft=BK-17) (no commitear test que pegue a DB compartida con creds en CI). Se documenta en la Spec Compliance Matrix como `manual:<evidence>`.

## Review Workload Forecast

Estimated: ~950 additions (excl. `public/openapi.json` generado) + ~40 deletions = ~990 total lines
400-line budget risk: ***High***
Chain strategy: ***size-exception*** (unidad transaccional cohesiva — separar migración/ruta/openapi deja estados intermedios no funcionales; precedente [https://jira.upexgalaxy.com/browse/BK-15#icft=BK-15](https://jira.upexgalaxy.com/browse/BK-15#icft=BK-15) ~1050 / [https://jira.upexgalaxy.com/browse/BK-17#icft=BK-17](https://jira.upexgalaxy.com/browse/BK-17#icft=BK-17) ~1750 single-PR)
