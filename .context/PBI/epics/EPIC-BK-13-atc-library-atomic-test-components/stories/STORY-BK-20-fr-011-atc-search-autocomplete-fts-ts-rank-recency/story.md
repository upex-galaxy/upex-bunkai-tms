# FR-011 — ATC search + autocomplete (FTS + ts_rank + recency decay)
...
As a tester composing a Test,

I want to search and autocomplete ATCs by title and tags within my workspace,

So that I can quickly find reusable components without scrolling deep module trees.
...
Scenario: Autocomplete returns top matches by ts_rank + recency
  ...
  And each item includes atc_id, slug, title, module_path, layer, and status_dot
...
- Ranking: relevance score = ts_rank(tsv, q) * exp(-age_days / decay_constant). Decay constant documented in code.
...
- Ranking expression: ts_rank(tsvector, plainto_tsquery(query)) combined with recency decay on updated_at
...
- Response shape: { items: [{ atc_id, slug, title, module_path, layer, status_dot }] }
...
The Test composition UI calls GET /atcs/search?query=...&module_id=... as the user types. The service layer validates the query, computes the workspace_id from the session, builds a SQL query that scores each ATC using ts_rank against the maintained tsvector column, multiplies by a recency decay factor based on updated_at, applies the module-subtree filter when present, orders DESC by combined score, and limits to min(requested, 50). The trigger on atcs keeps the tsvector column synchronized whenever title or tags change.
...
_Last sync: 2026-05-20T00:58:07.570Z_
