...
| Q3 | 100% auto-triggers finish? | Resolved as scope boundary | Benjamin: "not blocking, flagging for BK-39." `out-of-scope.md`: finishing is BK-024/BK-39. BK-35 advances progress only, never calls finish. |
| Q4 | Authorization | New decision, grounded in precedent | Verified live: `run*steps*insert*workspace*role*member*plus`/`run*atcs*insert*workspace*role*member*plus` (0031) gate on `bunkai*can*write*workspace` = `role in (member,admin,owner)` (0005*rls_helpers.sql:48) — same gate BK-36/39 use (`run:execute` scope, `lib/api/principal.ts:31`) and the same list `runs/[runId]/page.tsx:61` already derives `canAbort`/`canFinish` from. Reused as-is. |
...
| ATP | BK-40 data contract | Out of scope | `out-of-scope.md`: filing a bug is BK-40. BK-35 exposes step data in the existing `bunkai*run*json` shape; BK-40 designs its own consumption. |
...
2. ***No ****`runs.progress`**** column, none needed.**** `RunnerView.tsx:143-150` (already shipped, BK-34) computes `pct = done/total**100` client-side, `done` = any non-`pending` step. Generic enough already — zero changes required; satisfies AC3 verbatim.
...
`runs` deliberately excluded — BK-35 never mutates a `runs` row. Default `REPLICA IDENTITY` (PK) suffices; NEW row values are always included regardless. Split from DB-1 for independent review, mirrors BK-38's DB-1/DB-2 split.
...
| Progress % (AC3) | TS, `RunnerView.tsx` (BK-34, unchanged) | none added | Not new logic (finding 2) — pre-existing, generic, needs no modification. |
| Mark-body validation | TS, `validation.ts` | existing `validation.test.ts` (modify, add cases) | Genuine pure-function unit, same file BK-36/39's schemas live in. |

***Reconciliation with the briefing***: it suggested mirroring `duration.ts` for BOTH verdict-derivation and progress-calc. Progress needed zero new code (finding 2). Verdict-derivation stays SQL + DB-integration test per this repo's own stated precedent (BK-38) — a considered departure, not a silent override.
...
- ***D6***: Progress is 100% client-derived, pre-existing BK-34 code — no server storage/computation.
...
- ***D-UI-2***: §4.5's tracked status ("0%, build from scratch... Impl: none") is stale — contradicted by the already-substantial live `RunnerView.tsx` (BK-34/36/39). Flagged for a future `/design-system` or `/sync-ai-memory` pass; non-blocking.
- ***D-UI-3***: Report-bug drawer (mockup) is BK-40 territory, not touched here.

## Dependencies

- [x] BK-34 — `runs`/`run*atcs`/`run*steps` schema, `bunkai*create*run`, base `RunnerView.tsx` confirmed present by direct read.
- [x] BK-36/BK-39 — `bunkai*abort*run`/`bunkai*finish*run`, their routes and modal UI confirmed present by direct read — the patterns this plan mirrors throughout.
...
Decision trace: Q1=No (new domain logic -- a new RPC, a new API endpoint, a genuinely new realtime subscription module, and a substantial UI addition -- not a rename, formatter run, codegen, or vendor bump) · Q2=No (the natural 4-layer decomposition is DB [migrations 0042+0043 ~210 lines + their own mark-step.test.ts ~230 lines = ~440 combined], API [route pair + validation/errors/rpc touches, ~200 lines], Realtime [realtime-run-channel.ts + test, ~190 lines], UI [RunnerView.tsx addition, ~280 lines) -- API/Realtime/UI each individually clear the 400-line ceiling on their own, but DB's migration and its own co-located test combine to ~440, just over budget; artificially splitting the migration from the test it verifies into two separate chain slices would satisfy the line-count metric while violating this repo's own established "tests stay with the behaviour they verify" convention (git-flow-master Step 3.2) for no real review-quality gain -- so the natural DB slice stays as one unit, over budget) · Q3=Yes (two independent shared-scaffolding signals: `bunkai_mark_run_step` is the RPC contract the API route and, indirectly, the UI both consume, mirroring BK-38's own RPC-scaffolding case; more significantly, `lib/runs/realtime-run-channel.ts` is the FIRST real-time primitive in this entire codebase per ADR-0010's own text ("no existing test/mocking pattern for it") -- the UI slice cannot be meaningfully validated until this genuinely new pattern is proven correct, so merging layers piecemeal to `staging` would expose an unvalidated new primitive before the slice that actually exercises it lands) → feature-branch-chain
Decided by: /git-flow-master §Chained-PR decision tree (branching-strategies.md)
Decision needed before apply: No

***Branch plan******:*** integration branch `feat/BK-35-mark-run-step` cut from `staging`. Child 1 = DB layer (migrations 0042+0043, mark-step.test.ts) → merges into the integration branch. Child 2 = API layer (route pair + validation.ts/errors.ts/rpc.ts touches) → merges into the integration branch. Child 3 = Realtime layer (realtime-run-channel.ts + its test) → merges into the integration branch. Child 4 = UI layer (RunnerView.tsx wiring, the dominant cost driver) → merges into the integration branch. Final PR = integration branch → `staging`. Mirrors BK-38's own chain shape (DB → API → UI), with Realtime inserted as its own slice given it is new, unprecedented infrastructure in this codebase, not a reuse of an existing pattern.
