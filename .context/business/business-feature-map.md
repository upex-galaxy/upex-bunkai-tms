...
> **Last update**: 2026-08-24 (auth / API-inventory / shipped-flag corrections against the live code)
> **Sources**: `.context/PRD/mvp-scope.md` (9 epics, 38 user stories), `.context/SRS/functional-specs.md` (40 FRs), `public/openapi.json` (the LIVE generated OpenAPI 3.1 contract — `.context/SRS/api-contracts.yaml` is the design-time INPUT only, per `.context/SRS/README.md` "Notes", and never tracked the build), `lib/api/route-capability-coverage.snapshot.json` (auth-posture census), `.context/SRS/architecture-specs.md` (entity quick reference + RLS summary), `.context/PRD/executive-summary.md` (non-goals), `.context/PRD/user-personas.md` (Elena / Mateo / Sara / Karim), `.context/business/business-model.md` (Community / Cloud / Enterprise gating), `DESIGN.md` (component vocabulary), `.context/PRD/user-journeys.md` (UI flow inventory).
...
| -------- | ----------------------------------------------------------- | --- | ------------------- | --- | ------------------------------ | --------------- |
| FEAT-001 | Email + password sign-up / sign-in (primary rail, mandatory email-OTP confirmation); OAuth + magic-link as separate rails | 1.1 | {{PROJECT_KEY}}-001 | yes | Community / Cloud / Enterprise | Elena, Sara     |
...
**Capabilities:**
- [x] **Email + password is the PRIMARY auth method** (ADR-0007, BK-166) — an email-first flow drives `check-email` → `signin` → `signup` → `confirm` → `resend` (`app/api/v1/auth/*`, `app/(auth)/login/email-first-form.tsx`)
- [x] Mandatory email-OTP verification before any session or PAT is minted — no public auto-confirm (ADR-0007)
- [x] OAuth providers: GitHub, Google — a **separate rail** added later (ADR-0008, BK-3; `app/auth/oauth/[provider]/route.ts`, `app/auth/callback/route.ts`), not the primary method
- [x] Magic-link email auth (Supabase Auth in Cloud; Better Auth in Community Phase 2) — a third rail, `POST /api/v1/auth/magic-link`
...
- [x] Module nesting with circular-parent guard (422 `validation_failed`, `details.reason = "move_cycle"` — there is no `MODULE_CIRCULAR_PARENT` code)
...
**Capabilities:**
- [x] Token issuance via `POST /api/v1/tokens` — **cookie-session only**, a Bearer PAT is rejected with 403 ("a PAT must not mint a PAT", ADR-0001); plain token shown ONCE, SHA-256 hash stored in the sibling `access_token_secrets` table
- [x] Token capabilities (the full vocabulary, `lib/api/capabilities.ts`): `atc:read`, `atc:write`, `run:execute`, `workspace:admin` (+ optional `workspace_id` constraint). There is no `read` / `write` scope pair
- [x] Token revocation immediately invalidates (`DELETE /api/v1/tokens/{id}` — also cookie-session only)
- [x] Response shape: **bare-JSON success bodies**; failures use `{ error: { code, message, details?, request_id? } }` with snake_case codes (`lib/api/error-envelope.ts`). There is no `success` field and no `data` wrapper
- [x] URL-prefixed versioning `/api/v1` → `/api/v2` for breaking changes
- [ ] Rate limits: 100 req/min/token (writes), 600 req/min (reads), `Retry-After` on 429 — **NOT shipped**. No app-level limiter exists; the only 429s are passthroughs of Supabase's own auth throttling. ADR-0007 point 5 files the dedicated limiter as a documented follow-up (separate story)
- [ ] CLI commands: `bunkai auth login`, `bunkai atc list`, `bunkai run start` — **NOT shipped**. No CLI package exists in the repo
...
| #        | Feature                                                                                   | US      | FR                  | MVP | Editions                       | Primary persona |
| -------- | ----------------------------------------------------------------------------------------- | ------- | ------------------- | --- | ------------------------------ | --------------- |
| FEAT-044 | Idempotency-Key header (opt-in per endpoint; adopted by POST /tests + POST /runs)         | (infra) | {{PROJECT_KEY}}-037 | yes | Community / Cloud / Enterprise | Karim           |
...
> **The live inventory is `public/openapi.json`** (regenerate with `bun run openapi:gen`), and the live auth posture per route+method is `lib/api/route-capability-coverage.snapshot.json`. The tables below are the original design-time synthesis and have drifted; §4.1 is corrected because its auth column actively misled agents.

**Auth is not "Bearer everywhere except `/openapi.json`".** Every route declares one of **four postures** in `withApiHandler` (ADR-0001, `lib/api/handler.ts`); `middleware.ts` authenticates nothing on `/api/v1`:

| Posture               | Meaning                                                        | Count (method×route) |
| --------------------- | -------------------------------------------------------------- | -------------------- |
| `auth: 'required'`    | Authenticated + holds every listed capability                  | 67                   |
| `auth: 'authenticated'` | Authenticated, no capability required (must state `why`)      | 9                    |
| `auth: 'public'`      | No identity resolved                                           | 8                    |
| `auth: 'cookie-only'` | Authenticated, **Bearer PAT rejected with 403**                | 4                    |
| (`bypass`)            | Outside the gateway (spec serving, OAuth callback, `OPTIONS`)  | 4                    |

The **four `cookie-only` routes** are the surprising ones — a valid PAT gets a **403, not a 401**: `POST /api/v1/tokens`, `DELETE /api/v1/tokens/{id}` (a PAT must not mint or revoke a PAT), `POST /api/v1/me/active-workspace`, `DELETE /api/v1/workspaces/{id}/membership`.

The **public set**: `POST /api/v1/auth/check-email`, `/auth/confirm`, `/auth/magic-link`, `/auth/resend`, `/auth/signin`, `/auth/signup`, `GET /api/v1/health`, `GET /api/v1`. The **bypass set**: `GET /api/openapi.json`, `OPTIONS /api/v1`, `GET /auth/callback`, `GET /auth/oauth/{provider}`. Counts drift with every route — read the snapshot file, not this table.

### 4.1 Auth + Tokens (tags)

| Method | Endpoint                   | Purpose                                              | Auth (live posture)                           | FR                    |
| ------ | -------------------------- | ---------------------------------------------------- | --------------------------------------------- | --------------------- |
| GET    | `/api/openapi.json`        | Self-describing OpenAPI 3.1 spec                     | `bypass` (no gateway)                         | {{PROJECT_KEY}}-033   |
| POST   | `/api/v1/tokens`           | Issue Personal Access Token (plain token shown ONCE) | **`cookie-only`** — Bearer PAT gets **403**   | {{PROJECT_KEY}}-034   |
| GET    | `/api/v1/tokens`           | List my tokens                                       | `authenticated`                               | {{PROJECT_KEY}}-034   |
| DELETE | `/api/v1/tokens/{id}`      | Revoke a token                                       | **`cookie-only`** — Bearer PAT gets **403**   | {{PROJECT_KEY}}-034   |
| GET    | `/api/v1/me`               | Current authenticated user + active workspace        | `authenticated`                               | (cross-cut auth flow) |
| POST   | `/api/v1/auth/check-email` | Email-first routing probe (ADR-0007)                 | `public`                                      | {{PROJECT_KEY}}-001   |
| POST   | `/api/v1/auth/signin`      | Password sign-in (mints a default-scoped PAT)        | `public`                                      | {{PROJECT_KEY}}-001   |
| POST   | `/api/v1/auth/signup`      | Password sign-up (OTP confirmation required)         | `public`                                      | {{PROJECT_KEY}}-001   |
| POST   | `/api/v1/auth/confirm`     | Confirm the email OTP                                | `public`                                      | {{PROJECT_KEY}}-001   |
| POST   | `/api/v1/auth/resend`      | Resend the confirmation OTP                          | `public`                                      | {{PROJECT_KEY}}-001   |
| POST   | `/api/v1/auth/magic-link`  | Magic-link rail                                      | `public`                                      | {{PROJECT_KEY}}-001   |
...
| Settings — Tokens                      | `/{workspace_slug}/settings/tokens`                      | Table (tokens list), Button (issue), Modal (plain token shown ONCE), Button (danger "Revoke")                                                                                        | `POST/GET /api/v1/tokens` · `DELETE /api/v1/tokens/{id}`                                                   | {{PROJECT_KEY}}-034                                                                                                          | Elena, Karim    |
...
| Token issue          | `POST /api/v1/tokens`           | Zod                      | scopes (`atc:read` / `atc:write` / `run:execute` / `workspace:admin`) |
...
| Issue token modal     | Modal          | Settings → Tokens → "Issue token"      | `POST /api/v1/tokens` (cookie-only)         |
| Revoke token confirm  | Modal (danger) | Token row → "Revoke"                   | `DELETE /api/v1/tokens/{id}` (cookie-only)  |
