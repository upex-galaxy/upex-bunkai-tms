...
Appended per the append-only rule (`.context/ADR/README.md:43`) — nothing above this line is edited. The decision stands; these notes correct facts that have drifted since 2026-06-08, so this ADR read standalone does not mislead.

- **The `requires: []` KNOWN LIMITATION (frontmatter, `:14`) is obsolete — the escape hatch it describes no longer compiles.** `requires` is typed `NonEmpty<Capability>` (`lib/api/handler.ts:44` defines it, `:62` uses it), so an empty list is a red `types:check`. Coverage today, read from the committed `lib/api/route-capability-coverage.snapshot.json` (92 handler entries): **67** declare `auth: 'required'` with at least one real capability (`atc:read` 31, `atc:write` 27, `run:execute` 4, `workspace:admin` 5), **9** use the current escape hatch `auth: 'authenticated'` (each carrying a mandatory `why` sentence), 8 are `auth: 'public'`, 4 are `auth: 'cookie-only'`, and 4 bypass the gateway. A narrowly-scoped PAT no longer gets full member access across the authoring surface.
- **`:136-140` records `auth: 'required'` as "the default". There is no default, and the shipped outcome is STRONGER than what was decided here.** `WithApiHandlerOptions` (`lib/api/handler.ts:50-62`) is a closed union of four postures with no optional field, so a route handler cannot compile without naming one (`lib/api/handler.ts:46-49` states this intent in code). Secure-by-default became state-your-posture-or-you-do-not-compile: forgetting is a local type error, not a permissive production endpoint.
- **`:94`'s public-auth list (signin / signup / magic-link) is stale — there are now 6 public auth routes.** `check-email` and `confirm` arrived with [ADR-0007](./ADR-0007-password-auth-and-email-otp.md) (BK-166) and `resend` with BK-181; all six carry `auth: 'public'` in `lib/api/route-capability-coverage.snapshot.json` and all six are published in `public/openapi.json` (`/api/v1/auth/{check-email,confirm,magic-link,resend,signin,signup}`). **Read `public/openapi.json` as the live inventory** rather than this table — the list has already drifted twice.
- **`:101`'s deliberate cookie-only parity exception for `invites/accept` POST was dropped; the follow-up at `:211` ("invite-accept = verify") is resolved as "no exception".** The route is `auth: 'authenticated'` (`app/api/v1/invites/accept/route.ts:138`), so a Bearer PAT is accepted. It requires no capability because the caller is not yet a member of the target workspace when accepting, so no in-workspace capability check can apply — the reason is recorded inline at `app/api/v1/invites/accept/route.ts:139`. The token-mint exception at `:100` is unchanged: `app/api/v1/tokens/route.ts:108` is `auth: 'cookie-only'`, and `tokens/[id]` DELETE likewise per the snapshot.
- **The §4 verification layer exists but under different names, and one promised piece is still missing.** Committed today: `lib/api/route-posture-scan.ts` plus `lib/api/route-posture-scan.test.ts` (static scan of every handler's declared posture; hardened under BK-542 to report undeclared handlers instead of throwing — `lib/api/route-posture-scan.test.ts:70-114`), `lib/api/route-capability-coverage.test.ts` asserting that scan against the committed `lib/api/route-capability-coverage.snapshot.json` (`:94` matches the snapshot, `:109` leaves no handler without a posture, `:122` requires a justification on every no-capability posture, `:136` requires at least one capability wherever the posture is `required`), and the two env-guarded runtime tests `lib/api/rls-parity.test.ts` (cross-tenant isolation of the impersonating client) and `lib/api/auth-coexistence.test.ts` (cookie ⊥ PAT independence, added by ADR-0007 / BK-166). What does **not** exist is the literal §4 harness at `:146-148` — a parameterized test driving the route catalog with both a cookie and a PAT and asserting equivalent responses. Treat that part of §4 as still open.
