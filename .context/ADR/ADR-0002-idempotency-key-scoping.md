...
5. **In-flight and failed rows are race-safe.** A `pending` row with the same key + hash means the original request is still in flight → 409 `conflict` (retry shortly) instead of a concurrent duplicate write. A `failed` row is reclaimed via an atomic compare-and-set (`UPDATE … WHERE status = 'failed'`); losing that race → 409. A concurrent first-time insert losing on the unique constraint (23505) → 409. Handlers `discardIdempotencyResult` only on pre-commit failure; once the business write commits, a snapshot-store failure leaves the row `pending` (expires with the TTL) and the handler still answers success.
