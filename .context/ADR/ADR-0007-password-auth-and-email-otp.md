...
- **A further public auth route joined the family after this ADR and no ADR covers it: `POST /api/v1/auth/resend` (BK-181).** Its posture is `auth: 'public'` (`app/api/v1/auth/resend/route.ts:62`), and it is published in `public/openapi.json`. That makes the complete public auth surface **six** routes — `check-email`, `confirm`, `magic-link`, `resend`, `signin`, `signup` — all recorded as `public` in `lib/api/route-capability-coverage.snapshot.json`. Noted here so the family inventory is complete; it changes no decision above.
- **The four security-review follow-ups at `:60-63` still read true.** The first (`:60`) was already recorded as resolved when written and remains so: `DEFAULT_PAT_SCOPES` + `assertNoGlobalAdminScope` run on both minting rails (`app/api/v1/auth/confirm/route.ts:51-52`, `app/api/v1/auth/signin/route.ts:40-41`; helper at `lib/api/pat.ts:22-38`). The other three are genuinely **OPEN**: no app-level rate limiter exists anywhere under `app/` or `lib/` (a grep for `rateLimit` / `rate-limit` returns nothing), so the `check-email` enumeration surface is still throttled only by whatever Supabase enforces; `lib/api/logging.ts:18-33` logs request metadata only and implements no field scrubbing, so the "scrub `refresh_token` / PAT from logs" item is unaddressed rather than moot; and the PAT hash comparison is still a plain `!==` (`lib/api/middleware/bearer.ts:93`) under a comment that describes it as a constant-time compare (`lib/api/middleware/bearer.ts:10-11`).
