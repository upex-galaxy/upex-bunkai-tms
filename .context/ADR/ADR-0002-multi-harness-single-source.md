...
5. **Per-host MCP files with semantic parity.** `.mcp.json`, `opencode.jsonc` and `.codex/config.toml` each declare the same server set in their native format; the canonical set is whatever `.mcp.json` declares (`context7`, `tavily`, `supabase`, `n8n` out of the box). Parity is checked by normalizing the three formats and comparing the `.env` variables each server depends on, so a server added to one host only fails. The four boilerplate-known ids additionally get a strict per-host shape check when present; any other server gets the generic check only, so a downstream project may change its set freely. Codex cannot expand `${VAR}` in `args`, so its adapter uses HTTP with `bearer_token_env_var` for `tavily` and env-only auth for `supabase`. Gemini CLI and Cursor stay at the template level (`docs/mcp/*.template.*`), no runtime adapter.
