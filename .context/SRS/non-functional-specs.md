...
| Concern                        | Approach                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Authentication**             | Supabase Auth for end-users, with **email + password as the primary rail** (`signInWithPassword` in `POST /api/v1/auth/signin`) and a mandatory 6-digit email OTP on sign-up (`POST /api/v1/auth/confirm`, ADR-0007); magic link, GitHub OAuth and Google OAuth are secondary rails. Bunkai Personal Access Tokens (Bearer `bk_pat_*`, sha-256-hashed at rest in the service-role-only `access_token_secrets` table) for API/CLI.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **Session management**         | Supabase JWT, 1h access token, 30d refresh token, sliding renewal; `middleware.ts` refreshes the cookie session on every matched request. Sign-out is client-side `supabase.auth.signOut()` (`components/providers/auth-context.tsx`), so revocation is performed by Supabase Auth — Bunkai owns **no** logout route under `app/api/`. **Target (not implemented):** an app-owned sign-out endpoint that also revokes the caller's PATs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Password policy**            | Magic-link primary; if password fallback enabled later, min 12 chars + zxcvbn score ≥ 3.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **API rate limits**            | ⚠️ **TARGET, NOT IMPLEMENTED.** No app-level rate limiter exists in `app/` or `lib/`. The only 429s today are pass-throughs of Supabase Auth's own throttle on the auth routes (`signup`, `confirm`, `resend`, `magic-link`, `check-email`), re-emitted as the `rate_limited` error code. Intended once built: 100 req/min/token for writes, 600 req/min for reads, `Retry-After` on every 429, token-scoped per workspace rather than global.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **Documentation**: README, OpenAPI spec, ADRs (`.context/ADR/`, indexed in `.context/ADR/README.md`) for stack-level decisions, ATC authoring guide.
- **Observability**:
