...
**The corrected systemic reading.** The earlier conclusion claimed both remaining stories were undeliverable because they need credentials an unattended run cannot provision. That was **half right and it generalised too fast from one example**. It holds for BK-43, whose per-workspace third-party secrets model is genuinely absent. It was false for BK-50 — where the credential requirement was an artifact of a mistaken plan, and evaporated the moment the plan was checked against code. **The pool is 1 (BK-43), not 0, and "needs infrastructure we lack" is a claim to verify against the codebase before it is used to close a run**, exactly as it turned out to be worth verifying for R2.
