...
1. Call `EnterWorktree({ name: "avalanche-worker-b" })`. This creates your worktree under
   `.claude/worktrees/avalanche-worker-b/`, branches it from `origin/staging` (verified: this repo's
...
4. Dev server is OPTIONAL for this run — live-UI validation is suspended (§9b), so you don't need
   one running by default. Start `bun run dev -- -p 3002` yourself only if you personally want to
...
5. When your entire batch (initial + anything pulled from the shared queue) is fully done and merged,
   ...
   `git worktree remove .claude/worktrees/avalanche-worker-b` from the main checkout instead.
```
