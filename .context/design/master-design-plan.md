...
Required: welcome banner ("Welcome back, {name}") ✅ **shipped (BK-255)**, 4 stat cards (TOTAL ATCS / ACTIVE RUNS / OPEN BUGS / COVERAGE w/ deltas), Recent projects list (coverage `.bar` progress + module/ATC counts + last activity), Recent activity feed (last-24h, actor + action + target + time), Active test runs table (Run/Project/Mode/Status/Progress/Executor), header actions Sync / Filter / Start run.
