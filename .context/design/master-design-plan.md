...
### 4.2 Home / Dashboard — 🔶 (banner + 4 of 5 widgets shipped)
Mockup: `screens/home.jsx` · Impl: `app/(app)/home/page.tsx` + `components/home/WelcomeBanner.tsx` (BK-255)

Required: welcome banner ("Welcome back, {name}") ✅ **shipped (BK-255)**, 4 stat cards (TOTAL ATCS / ACTIVE RUNS / OPEN BUGS / COVERAGE w/ deltas), Recent projects list (coverage `.bar` progress + module/ATC counts + last activity), Recent activity feed (last-24h, actor + action + target + time) ✅ **shipped (BK-260)** — `components/home/RecentActivity.tsx`, a presentation layer over BK-49's feed with no endpoint of its own; placement + relative time recorded as §5 **D22**, Active test runs table (Run/Project/Mode/Status/Progress/Executor), header actions Sync / Filter / Start run. The OPEN BUGS stat card is ✅ **shipped (BK-258)** — `components/home/OpenBugs.tsx`, backed by `GET /api/v1/workspaces/{id}/open-bugs`; the four-up KPI grid, drill-through, `P3+` bucket, sprint delta, and what "open" counts are recorded as §5 **D23**.
**Struck from the contract:** the sprint-day eyebrow ("SPRINT 24-Q2 · DAY 7/10") — no Sprint/iteration entity exists in the product and none will be designed for it; ratified via proposal P-2026-08-02-03 on BK-261 (2026-08-02) and recorded as §5 **D20**. The eyebrow's position in the layout now carries the active workspace's name instead (same departure entry).
**Remaining widget:** BK-259 (Coverage stat card) only — runs (BK-30), bugs (BK-31) and coverage (BK-44) all shipped at the data layer; BK-256, BK-257, BK-258 and BK-260 are done. **Priority: P2.**
...
| | BK-257 Recent projects with activity and status | **Home** recent-projects list | §4.2 · `home.jsx` |
| | BK-258 Open bug count + severity breakdown | **Home** stat card (OPEN BUGS) | §4.2 · `home.jsx` |
| | BK-259 Workspace test-coverage summary | **Home** stat card (COVERAGE) | §4.2 · `home.jsx` |
| | BK-260 Condensed recent-activity feed | **Home** recent-activity feed (condensed — the paginated standalone feed is `/activity`, D15) | §4.2 · `home.jsx` · §4.16 |
