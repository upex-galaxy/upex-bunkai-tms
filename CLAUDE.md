...
13. **STRUCTURED FEEDBACK + LONG REPORTS — WOKITOKI (`toki`)**: Two triggers, both PREFER `toki` (not mandatory — AI judges per reply):
    - **(a) Multi-point feedback** — reply needs user to react to **>3 decision points at once** (mix single/multi/toggle), feedback **anchored to an exact phrase** (highlight-to-quote), or an **image pasted back**.
    - **(b) Long report delivery** — whenever you'd otherwise dump **more than ~2-3 paragraphs (or 2 big paragraphs) of information into the terminal chat**, strongly consider delivering it as `toki` report blocks instead. Rationale: the SAME content you'd paste in chat goes into the spec `content` (markdown) — **zero information loss** — but broken into per-block paragraphs the user can react to one at a time (per-block text + highlight-to-quote). Turns a one-way wall of text into a two-way exchange and maximizes the user's ability to respond precisely. Report blocks = `blocks` with NO `controls`; mix with control blocks for a hybrid.
    - **Why over alternatives**: `AskUserQuestion` is capped ~4×4, no rich free-text, can't show reference content while answering; inline prose questionnaires produce unanchored replies the AI must guess-map back.
    - **Mechanics**: skill `wokitoki` is user-level (auto-loads every session); binary is global; all output lands in `~/.toki/` — **zero repo footprint**. Flow: write spec JSON → `toki <specPath>` (blocking) → parse stdout Result JSON same turn.
    - **Keep it cheap**: `AskUserQuestion` for 1-2 simple picks / single yes-no; plain chat for short answers; plain output when non-interactive (CI / no human at a browser).
