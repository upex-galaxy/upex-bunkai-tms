...
//      link schemes from the Markdown *source* before persisting, so the stored
//      text is clean (AC: "a pasted script is stripped on save", "the javascript
//      link has been removed"). It is intentionally NOT an HTML sanitizer:
//      running sanitize-html over Markdown corrupts legitimate content — e.g. an
//      inline-code comparison `a < b` becomes `a &lt; b`, and Markdown-syntax
//      links `[x](javascript:...)` are missed entirely. So we clean the Markdown
//      directly and leave code/text byte-for-byte intact.
...
// GFM autolink with an unsafe scheme: <javascript:...>.
const UNSAFE_AUTOLINK = /<\s*(?:javascript|data|vbscript|file):[^>]*>/gi;
// Markdown inline link / image: optional `!`, [text], (url ...optional title).
// The URL matcher tolerates one level of balanced parentheses so a scheme like
// `javascript:alert(1)` is captured whole rather than truncated at the inner `(`.
...
function stripUnsafeMarkdownLinks(input: string): string {
  return input.replace(MD_LINK, (match, _bang: string, text: string, rawUrl: string) => {
    ...
    if (HAS_SCHEME.test(url) && !SAFE_SCHEME.test(url)) {
