...
├── business/             # Constitution + maps (data, feature, api) + domain-glossary.md (canonical terminology)
├── PRD/                  # Product Requirements
├── SRS/                  # Software Requirements
└── PBI/                  # Jira-synced cache (epics/stories, bugs, tech-stories, tests…) — regenerated by jira:sync-issues
packages/
...
bun run jira:sync-issues  # Sync Jira issues -> .context/PBI/ (epics, stories, bugs, comments; pull / get / jql)
bun run jira:sync-workflows # Sync Jira statuses/transitions -> .agents/jira-workflows.json
bun run jira:sync-link-types # Sync Jira link types -> .agents/jira-link-types.json
