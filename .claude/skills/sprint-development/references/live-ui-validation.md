...
Drive the running dev server with a scripted login. The account is the declared automation identity (§0 + `live-ui-identity.md`); its values come from `.env` at runtime (CLAUDE.md Critical Rule #1) — never hardcode. Env URL comes from `.agents/project.yaml` (`{{WEB_URL}}` for the active env; localhost dev server for the real-time check).
