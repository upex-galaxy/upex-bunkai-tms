...
- **A published ruling that names what to create is EXECUTED, not escalated — by whichever mode finds it.** When a ticket already carries a scored, attributed ruling that specifies concrete follow-on work (split this story into these three, re-parent these tests, correct this field), that work is data entry against a settled decision. **Dispatch `/product-management` and do it, in ANY mode, then continue.** It does not consume the story/bug cap — the cap counts stories IMPLEMENTED, not tickets administered. Never ask a human who should execute an already-made decision. The line this rule exists to draw: *authoring* refinement or product intent that does not exist is `discovery`'s job and is gated; *materializing* a ruling already written on the ticket is nobody's exclusive job and is not gated. Measured cost of getting this wrong: **eight consecutive empty `story` runs** over roughly ten minutes of backlog data entry, the last of which escalated "who should do this" to the operator instead of doing it.
- **A defect this run itself reproduced and root-caused is FILED, not carried forward unticketed — but it is NEVER worked in the same run that files it.** Absence of a tracker record is NOT a blocker: `CLAUDE.md` Rule #18 enumerates exactly two and "no key exists" is neither. Filing is permitted in ANY mode, consumes no cap, and is gated by a **citation, never an assertion** — all four required, and a missing or unquotable citation on ANY of them IS the blocking value (record it and file nothing): (i) a reproduction, either already recorded by a PRIOR run in the escalation log or executed live this run with the command and its raw output pasted; (ii) **two differently-worded live backlog searches returning nothing**, quoted — on 2026-08-13 an item called "un-ticketed since 08-07" turned out to be BK-249, filed by hand on 07-31, with the error propagated to four documents; (iii) a root cause at a specific `file:line` in this repo; (iv) the expected behaviour **quoted from an existing artifact** (acceptance criterion, spec line, glossary entry, documented contract) and NEVER composed — if it cannot be quoted, the question is a product question and goes to the `AI PO/BA` per `decision-protocol.md` §5.1, not into a filing. Cap: **2 filings per run.** Filing authors no product intent, which is why it is not `discovery`-gated: a defect record asserts a gap against intent someone else already published, whereas shift-left refinement SETS the intent — the workflow catalogue encodes the difference, stories carry a `shift_left_qa` status and `bug`/`defect` do not.
- **A filed defect lands `open`, UNASSIGNED, labelled `ai-filed`, with its AI authorship in the FIRST line of the description — not only in a comment.** The Reporter field will read as the operator's own human account because the tracker CLI authenticates as the operator, so that field actively misrepresents who found the defect and cannot be corrected; the disclaimer has to sit where it cannot be scrolled past. Severity and priority are **provisional, with a per-field justification**, never asserted flatly — they are normative product judgements smuggled into an evidential record. `root_cause`, `fix` and `workaround` stay EMPTY (`jira-required.yaml` defines `root_cause` as set AFTER the fix). Re-fetch and clear Story Points: an "Automation for Jira" rule silently sets 1.0 with no changelog entry. Assigning a shift-left owner is fabrication — a self-filed ticket's comment trail contains only the AI's own comments. **The run report separates FILED from SHIPPED in its headline; a run reporting "0 shipped, 2 filed" is an empty run and is read as one.**
...
**The "Dispatches to" column names each mode's PRIMARY callee, not its only permitted one.** Every mode may additionally dispatch `/product-management` to materialize a ruling already published on a ticket — that is execution of a settled decision, it consumes no cap, and it needs no approval. What the `discovery` row's "only after synchronous approval" gate covers is *authoring new product intent*: proposing work the product does not yet have a decision about. Read as a prohibition on `story`/`bug` ever calling `/product-management`, this table has already cost eight consecutive empty runs.
...
4. **Readiness is not status — but an unresolved question is work, not a rejection.** Read the refinement trail before accepting a candidate; never infer readiness from the status field. A ticket can be moved to ready-for-dev with a declared blocker still open and no comment trail explaining the move. What you do about it depends on WHICH of the two cases you are in, and conflating them is the failure mode this step exists to prevent:
   ...
   - **(c) The trail contains a published ruling that specifies follow-on ticket work which nobody executed** — most often "this story is oversized, split it into these three and re-parent its tests." **This is NOT a blocker and NOT an escalation. Execute it**: dispatch `/product-management` with the ruling as its specification, let it create and link what the ruling names, resync the cache, and then re-evaluate the candidate. It does not consume the mode's cap. Only if the candidate is STILL blocked after the ruling is materialized (typically because a successor now carries a real dependency) do you move on — and then you move on under (a), citing the dependency, not under "somebody should have done the paperwork."
...
- **Escalating is expensive here in a way it is not interactively.** There is nobody to answer. An escalation ends the run's forward progress until the next human touch, so an over-stop costs a whole scheduled slot. With `migrations: unrestricted` (Change 1 — see Configuration), the migration gate is no longer an escalation source at all: every migration class, including destructive ones, applies unattended. That leaves the escalate-only categories genuinely narrow, and product is NOT among them: a genuinely NEW security posture (applying an already-ratified pattern is implementation), and anything the operator explicitly reserved in-flow.
...
- **A ruling already published is executed, not re-escalated.** Deciding is one job; *materializing* what a decision named is a second one, and the second is where a run has actually stalled in practice. If the ticket carries a scored ruling saying "split this into three and re-parent the tests", dispatch `/product-management` and do it — any mode, no cap consumed, no human asked. Asking who should execute a settled decision is the same failure as re-asking a settled question, and it costs a whole scheduled slot each time.
...
**Escalation, when it does fire, is a clean stop, not a hang — but it must NOTIFY, not end silently.** For `story`/`bug` mode, in this order: (1) push anything unpushed, (2) write the escalation entry (what happened, why it is this category, what the human needs to decide, what the downstream cost of waiting is), (3) release the lock, (4) **notify — two calls, in order**: first `slack_send_message` to `escalation_channel` (`.agents/project.yaml` -> `autonomous_delivery.escalation_channel`, `#claude-routine-blockers`), then `PushNotification` second, (5) report, (6) end the run. Both notification calls are REQUIRED, not optional, and fire for every `story`/`bug` escalation — an escalation nobody is told about is indistinguishable from a run that silently died.

Why Slack is primary: it is an external channel that persists and leaves a record, and posting to it does not depend on the Claude client being connected or on Remote Control being reachable. A `PushNotification` that fails to deliver leaves no trace anywhere — nothing to check later, nothing for the next run's Phase 1 to find. The Slack message is markdown, so structure it (ticket key as a heading or bold lead, what is blocking it, what specifically the operator needs to decide) and keep it short enough to read on a phone; the `PushNotification` that follows stays the short one-line ping it always was (`status: "proactive"`, under 200 characters, one line, no markdown, leading with the ticket key and the actual decision needed, not "run stopped").

Failure behavior: if the Slack send fails, the run still sends the `PushNotification`, still writes the escalation-log entry, and still ends cleanly — a notification failure must never become a second failure that swallows the first. Record the Slack send failure in the run report so a human reading it later knows the primary channel was missed and to check the secondary one.

**`escalation_channel` carries escalations only.** A run that finishes cleanly does not post there, and neither does an empty run with nothing eligible — both of those are correct outcomes that belong in the run report, not in Slack. Posting a summary into `#claude-routine-blockers` would defeat the reason the channel exists: everything landing there is, by definition, something the operator must act on, and diluting that with routine "nothing to do" or "shipped fine" noise is exactly what turns a channel worth reading into one that gets muted.

Never park a scheduled session waiting on an answer that cannot arrive: notifying and waiting are different things, and only `discovery`'s approval gate below waits.
...
- **A18.** NEVER leave an escalation parked waiting for a reply, for `story` or `bug` mode. Push, write the entry, release the lock, post to `escalation_channel` via Slack (primary), send the `PushNotification` (secondary), report, end the run. (`discovery`'s approval gate is the one explicit, scoped exception to this — see Autonomy §Discovery's synchronous approval gate. Do not read that exception as license to park anywhere else.)
...
- **A22.** NEVER reimplement a stage that `/sprint-development` or `/product-management` owns. Dispatch it. **And note what this does NOT say**: it does not restrict WHICH mode may dispatch `/product-management`. Any mode may, to materialize a ruling already published on a ticket.
- **A27.** NEVER escalate, defer, or record-for-assignment a piece of ticket administration that a published ruling already specifies. Execute it — dispatch `/product-management`, create what the ruling names, resync, continue. "Who should do this?" is not a question for the operator when the *what* is already decided and written on the ticket.
- **A28.** NEVER carry a reproduced, root-caused defect forward as "unticketed" because no routine filed it — that is not a blocker under Rule #18, and it produced a pool that this project's own log counted as "the third" (2026-08-07), "the fourth" (2026-08-08) and "**five**" (2026-08-09). File it under the citation gate, or record which citation failed. And NEVER work a defect in the run that filed it: defects never pass shift-left, so their single human checkpoint is the QA retest — one agent observing, filing, setting severity, fixing, merging and transitioning leaves that gate existing on paper and empty in substance.
...
- [ ] Any published ruling specifying follow-on ticket work (split, re-parent, field correction) was EXECUTED via a `/product-management` dispatch this run — not escalated, not deferred, not recorded for assignment. Cap unaffected.
...
- [ ] Every `story`/`bug` escalation posted to `escalation_channel` via Slack first (ticket key, blocker, decision needed) and sent a `PushNotification` second (`status: "proactive"`, <200 chars, one line, no markdown, ticket key + decision needed first) before the run ended — never a silent stop; a Slack send failure logged in the run report, not skipped over
- [ ] `escalation_channel` carried escalations only this run — no run summary or empty-run notice posted there
...
- **Refinement, epics, acceptance criteria, backlog gaps** -> `/product-management`. **Any mode may dispatch it; `discovery` is not its only caller.** What is mode-restricted is AUTHORING new product intent — proposing what the product needs, which is `discovery`'s job and gated on its approval. **Materializing a ruling that is already published on a ticket is EXECUTION, and every mode may do it** (see the compact rule on published rulings). This line previously read "`discovery` mode's only callee" and was misread, on a real run, as a prohibition — a `story` run left an already-decided ticket split unexecuted and escalated "who should do this" instead. Defect records are a third case, distinct from both: they assert a gap against intent already published, so filing one authors nothing and is not `discovery`-gated — what stays gated is `discovery`'s unit of account, new user stories and epics (`discovery_definitions`), which a defect never consumes.
