...
> **Purpose**: Canonical, operational explanation of how this repo (Bunkai TMS — `upex-bunkai-tms`, built on `agentic-dev-boilerplate`) structures context so AI agents work effectively against it.
...
│   │   └── domain-glossary.md      Canonical domain terminology (ATC, KATA, IQL) — hand-maintained, wins over any other doc
...
│   └── PBI/                        Jira-synced cache (epics/stories, bugs, tech-stories, tests…) — generated by scripts/sync-jira-issues.ts
│
...
| `.context/business/domain-glossary.md`      | Canonical domain terminology (ATC = Acceptance Test Case, KATA, IQL); wins over memory and older docs |
...
| Jira content changed (issues, fields, comments) | `.context/PBI/` cache                         | `bun run jira:sync-issues` (pull / get / jql) |
| Domain term added or corrected            | `.context/business/domain-glossary.md`              | Edit glossary FIRST, then align docs + Jira   |
