...
│   ├── ADR/                        Architecture Decision Records (human-authored, append-only)
...
**Exception — `.context/ADR/`** is **human-authored by design** (no generator): Architecture Decision Records for important, hard-to-reverse decisions. Append-only — supersede, never delete. See `.context/ADR/README.md` for the when-to-write rule and template.
...
2. **Don't create new files under `.context/` without a generator** — they will drift and rot. Add the generator first. (Sole exception: `.context/ADR/` is human-authored by design.)
...
| `.context/ADR/README.md`                                                                     | Architecture Decision Records — when to write one, status lifecycle, index                 |
