...
2. **BOTH FILES OR NEITHER.** If the created story gets a `master-design-plan.md` §8 US->Screen row, the SAME change must also give it a `.context/dev-roadmap.md` entry — a §3 dependency row (an explicit "no incoming edge" is a valid row) and a §4 execution-sprint mention. Writing the §8 row alone is a defect, not a partial success: the story acquires a design contract and stays invisible to the execution order that is supposed to schedule it. If a run cannot write both, it writes neither and records why.
3. Append ONE entry per created artifact to `.session/autonomous-delivery/discovery/created-log.md`: key, title, parent, date, run session id, one-line reason. The file is APPEND-ONLY — never rewrite, resolve, or prune an entry.
4. Report the created keys to `report_channel`.

> **Why step 2 exists, recorded so it is not "simplified" back out.** This gap (`D-B` in the escalation log) reached its **SEVENTH** instance on 2026-08-31: BK-593/594 (08-22) -> BK-595/596 (08-23) -> BK-600/601 (08-24) -> BK-666/667 (08-28) -> BK-718 (08-30) -> BK-742/743 (08-30). Six consecutive `story` runs hand-patched the symptom into `dev-roadmap.md` and each recorded that the real fix belonged here, in this contract. Step 2 IS that fix. An eighth instance means step 2 did not hold, and the next thing to check is whether the registry was regenerated (`bun run skills:registry`) — an edit to this file reaches no subagent briefing until it is.
