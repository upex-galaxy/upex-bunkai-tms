...
# Sync AI Memory — Repo Doc Audit & Sync (mode `sync`)
...
**Target**: `$ARGUMENTS` forwarded by the alias or given in the invocation (leave blank to operate on the current repo)

---

## Step 0 — Verify the cross-harness instruction contract (shim guard)

This repository has a fixed instruction topology. Do not auto-select another memory file, and never treat `CLAUDE.md` as a sync target:

| Surface | Contract |
|---|---|
| `AGENTS.md` | Canonical instruction body and the ONLY editable policy source. Every sync patches here. |
| `CLAUDE.md` | Exact one-line import shim: `@AGENTS.md` plus a final newline. Generated; never patched. |
| `.agents/skills/` | Canonical skill source |
| `.claude/skills` | Generated alias; never edit through this path |
| `.claude/commands/*.md`, `.opencode/commands/*.md` | Generated transport wrappers from `.agents/compatibility/command-aliases.json`; never patched |

Gate, in order:

1. `AGENTS.md` missing → STOP. Report and do not create it from a template.
2. `CLAUDE.md` is not byte-for-byte `@AGENTS.md` + newline (operational prose, extra sections, a second rule list) → that is **structural drift**, not a sync target. STOP, report the drift, and never propagate that prose into `AGENTS.md` or anywhere else. The fix is `bun run agents:compat`, decided by the user.
3. Both hold → continue. `AGENTS.md` is the target of every sync below wherever an older doc says "the AI memory file".
...
- **Does NOT rewrite docs from scratch.** README and AGENTS.md are the BASE; this skill edits them, never regenerates them.
- **Does NOT generate new documentation files.** Use `/project-foundation`, `/project-bootstrap`, or manual creation for new docs.
- **Does NOT generate business maps.** Those are owned by skill `project-context` (modes `data`, `features`, `api`, `master-plan`, `dev-roadmap`; aliases `/business-data-map`, `/business-feature-map`, `/business-api-map`, `/master-implementation-plan`, `/dev-roadmap`).
- **Does NOT touch the `CLAUDE.md` shim or any generated harness surface.** See Step 0: operational prose found there is structural drift to report, never to sync.
...
- **Does NOT rewrite skill reference files.** `.agents/skills/agentic-dev-core/references/behavioral-layer.md`, `typescript-patterns.md`, and the other 6 references are skill-internal — managed inside the `agentic-dev-core` skill directory, not by this skill. Sync validates they exist + cross-doc consistency only.
...
| `AGENTS.md`                               | `anchor`          | Canonical AI memory file: loaded natively by OpenCode and Codex, imported by Claude Code through the `CLAUDE.md` shim. The only instruction body a sync may patch.                        |
| `CLAUDE.md`                               | `compatibility-shim` | Must remain exactly `@AGENTS.md` + newline. Verified in Step 0, never patched.                                                                                                        |
...
| `docs/**/*.md` (per audit)                | `supplementary`   | Any other doc the Phase 1 audit qualifies (setup guides, MCP guides, workflows); patched for command/skill/path drift only                                                                |
...
Exclude paths (hard-skip — never scan or report):
  ...
  - .scratch/, .tmp/, .cache/
  - coverage/
  - .agents/skills/*/SKILL.md (managed by their own skill)
  - .agents/skills/*/references/* (skill-internal docs cache)
  - CLAUDE.md (compatibility shim, verified in Step 0 — never a sync target)
  - .claude/commands/*.md and .opencode/commands/*.md (generated transport wrappers)
  - Any file generated by skill project-context (business-data-map.md, business-feature-map.md, business-api-map.md, master-implementation-plan.md, dev-roadmap.md — owned by their generating mode)

Classification heuristic — a file qualifies if it matches ANY rule:

RULE 1 — All-caps root files (highest signal):
  Files at the repo root whose stem is all uppercase (README.md, AGENTS.md, CONTEXT.md, INSTALLER.md, etc.). CLAUDE.md matches the rule but is the shim: report it as compatibility-shim, not as a target.

RULE 2 — Referenced by AGENTS.md or any SKILL.md:
  Scan AGENTS.md and .agents/skills/*/SKILL.md for path mentions under docs/ or repo root. Any file mentioned by name (not just a directory) qualifies.
...
RULE 4 — High inbound reference density (≥ 3 cross-references):
  Scan .agents/skills/**/*.md and .agents/compatibility/command-aliases.json for file-path mentions. Harness wrappers are generated transport, not sources. Count per target. Files with ≥ 3 references qualify.
...
2. Force-add the always-included docs + 1 standalone HTML (skip ones not on disk; mark them `not present`).
3. Force-promote `README.md`, `AGENTS.md`, and `CONTEXT.md` to `CRITICAL` if they aren't already.
4. Deduplicate.
...
CRITICAL (3)
  ...
  - AGENTS.md                                    [always-included, R1, R2, R5]
...
EXCLUDED (informational)
  - CLAUDE.md (compatibility shim, verified in Step 0)
  - SKILL.md files (managed by their own skill)
  - .context/business/business-*-map.md (owned by skill project-context)
...
## Phase 3 — Confirm the AI memory file

There is no detection step in this repository: the memory file is `AGENTS.md`, fixed by the cross-harness contract verified in Step 0. Harness-specific files are consumers, never targets:

| Harness          | Reads                                   | Sync target? |
| ---------------- | --------------------------------------- | ------------ |
| Claude Code      | `CLAUDE.md` → `@AGENTS.md` import       | No (shim)    |
| OpenCode         | `AGENTS.md` natively                    | Yes          |
| Codex            | `AGENTS.md` natively                    | Yes          |
| Cursor / Copilot | templates only, no committed rule files | No           |

If a second memory file appears (`GEMINI.md`, `.cursor/rules`, `.github/copilot-instructions.md`, a `CLAUDE.md` with prose), do NOT merge it and do NOT pick it: flag it as structural drift in the Phase 7 report and let the user decide.

> Do NOT bootstrap from inside `sync-ai-memory`. Clone the full boilerplate repository — `AGENTS.md`, the `CLAUDE.md` shim, `.agents/skills/`, and the compatibility tooling ship together. Once the contract exists, re-run `sync-ai-memory` to align cross-doc facts.
...
| `/skill-name` references          | `ls .agents/skills/<name>/` — does the skill exist?                                                   |
| `/command-name` references        | `grep '"alias": "<name>"' .agents/compatibility/command-aliases.json` — is the alias declared?        |
...
**`AGENTS.md`** — current structure is the numbered sections of the live file (read it; never assume a count). Preserve:
...
Sections that may be **patched** (data drift only): §5 SKILLS + COMMANDS + MCPs REGISTRY (T1 skill table, slash commands table, MCP table — verify against `.agents/skills/`, `.agents/compatibility/command-aliases.json`, `.mcp.json`).
...
**Critical Rule #10 enforcement**: AGENTS.md must NOT inline build/test/lint script names in tables. Detect any `| \`bun run`/`| \`npm run`/`| \`pnpm run`table row — flag as STRUCTURAL drift (violates Rule #10, scripts must defer to `package.json`).
...
`docs/onboarding.html` is hand-maintained — there is no source MD to render from. Regenerating would destroy the hand-crafted sidebar nav, tab panels, JS-driven interactive navigation, card layouts, and CSS classes. Phase 4b patches **only text nodes** that mirror facts already drift-checked against the live repo state (`package.json` scripts, `.agents/skills/` skill names, `.mcp.json` MCP names, file paths, etc.).
...
2. **Collect drift facts** from Phase 4's MD patch log AND from a direct repo audit of the things the HTML quotes (script names from `package.json`, skill names from `.agents/skills/`, MCP names from `.mcp.json`, file paths). There is no source MD — drift discovery is per-HTML, not derived from a paired markdown.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Command names               | All targets                                                                                                     | `AGENTS.md` says `/refresh-ai-memory`, `docs/onboarding.html` still says `/sync-ai-memory`                                                                                                                                    |
| Skill names                 | All targets                                                                                                     | Skill renamed but not all docs updated                                                                                                                                                                                        |
| `.context/` directory paths | `AGENTS.md`, `README.md`, `CONTEXT.md`                                                                          | One doc says `.context/business/`, another says `.context/discovery/`                                                                                                                                                         |
| Environment URLs            | `AGENTS.md`, `README.md`                                                                                        | Staging URL changed in only one doc (note: AGENTS.md should NOT inline env URLs — they live in `.agents/project.yaml`)                                                                                                        |
| Script names                | `README.md`, `INSTALLER.md`, `docs/onboarding.html` (NOT `AGENTS.md` — Rule #10 forbids inlining scripts there) | Script renamed in `package.json` but not in docs                                                                                                                                                                              |
| Project identity            | `README.md` only (NOT `AGENTS.md` — lives in `.agents/project.yaml`)                                            | Name / stack / target repo mismatch                                                                                                                                                                                           |
| Instruction topology        | `README.md`, `INSTALLER.md`, `docs/*`                                                                           | A doc presents `CLAUDE.md` as the canonical memory file instead of the `AGENTS.md` source plus shim                                                                                                                           |
| Reference files exist       | `AGENTS.md` pointers vs `.agents/skills/agentic-dev-core/references/`                                           | AGENTS.md §2 points to `behavioral-layer.md`, §10 to `typescript-patterns.md`, §3 to `orchestration-doctrine.md` / `briefing-template.md` / `dispatch-patterns.md` / `skill-composition-strategy.md` — all must exist on disk |
| MCP table                   | `AGENTS.md §5` vs `.mcp.json`                                                                                   | MCP listed in one but not the other                                                                                                                                                                                           |
...
**Instruction contract**: `AGENTS.md` canonical, `CLAUDE.md` shim verified (Step 0)
...
| AGENTS.md                               | CRITICAL        | updated | +5 / -3       | §5 skill/MCP table      | section structure preserved    |
| CLAUDE.md                               | SHIM            | unchanged | 0           | —                       | exact `@AGENTS.md` shim verified |
...
1. **README + AGENTS.md current state IS the base.** Always read before writing. Patch against the read content. Never use `Write` for an existing file in this skill — only `Edit`.
...
5. **Scope is dynamic.** The always-included docs (+ 1 standalone HTML) are a floor, not a ceiling. The audit can extend the list; it cannot shrink the floor.
6. **No re-introduction of forbidden AGENTS.md sections.** Session Log, Known Issues, Discovery Progress, Next Actions, Future Hooks, Project Identity placeholder, Environment URLs placeholder, Quick Start, Onboarding — none of these belong in AGENTS.md. If the audit suggests adding any of them, flag as STRUCTURAL drift and refuse autonomously.
10. **The shim is never a target.** Any prose in `CLAUDE.md` is structural drift (Step 0): STOP and report; never patch it and never carry it into `AGENTS.md`.
...
When this skill regenerates or rewrites memory documents (AGENTS.md, CONTEXT.md, README.md sections it owns, onboarding HTML, docs/ pages it owns), prefer running `caveman-compress <file>` BEFORE writing the new content if caveman is installed user-level. Never run it on the `CLAUDE.md` shim. caveman-compress preserves code blocks, URLs, and paths byte-for-byte while compressing prose ~46% on average. This is a one-shot rewrite — re-runs are idempotent.

- Trigger: when the skill output is destined to be written to disk as a memory file.
- Skip when: the file is human-facing primary documentation that must stay verbose (e.g. CONTRIBUTING.md tutorial sections).
- Verification: after compression, the skill must re-validate that all variable placeholders, code fences, and reference links survived (caveman-compress is byte-preservation-aware but a final grep is cheap insurance).
...
## Excluded by default (never touched by this skill)

- `CLAUDE.md` — compatibility shim (Step 0)
- `.claude/commands/*.md`, `.opencode/commands/*.md` — generated wrappers (`bun run agents:compat`)
- `.agents/skills/*/SKILL.md` — managed by each skill's own author
- `.agents/skills/*/references/*` — skill-internal documentation caches
- `.context/business/business-data-map.md` — owned by `project-context` mode `data`
- `.context/business/business-feature-map.md` — owned by `project-context` mode `features`
- `.context/business/business-api-map.md` — owned by `project-context` mode `api`
- `.context/master-implementation-plan.md` — owned by `project-context` mode `master-plan`
- `.context/dev-roadmap.md` — owned by `project-context` mode `dev-roadmap`
- `.agents/skills/REGISTRY.md` — owned by `bun run skills:registry`
...
- [ ] Cross-harness contract verified: `AGENTS.md` present, `CLAUDE.md` is the exact shim (Step 0)
- [ ] Audit sub-agent dispatched (Phase 1)
- [ ] Audit list merged with the always-included docs + 1 standalone HTML (Phase 1)
- [ ] User confirmation received via approval gate (Phase 2)
- [ ] AI memory file confirmed as `AGENTS.md`; no second memory file merged (Phase 3)
...
- [ ] Rule #10 enforcement: AGENTS.md contains 0 inlined `| \`bun run` table rows
