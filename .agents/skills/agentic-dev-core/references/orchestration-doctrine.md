...
A dispatched subagent sees ONLY: its briefing, the `## Project Standards (auto-resolved)` block copied from `.agents/skills/REGISTRY.md`, and the files the briefing names. It does NOT walk the `references/` tree of the skills that own the work.
...
AGENTS.md Rule #10 ("scripts = read `package.json` directly") generalizes to ALL project configuration:
