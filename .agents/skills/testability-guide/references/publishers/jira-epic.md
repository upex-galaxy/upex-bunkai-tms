...
>
> **Tool**: `[ISSUE_TRACKER_TOOL]` per `AGENTS.md` §6. Resolves to `/acli` primary, Atlassian MCP fallback. This adapter owns the WHEN/WHAT — for the HOW (exact syntax, auth, flags, ADF), load `.agents/skills/acli/SKILL.md` §Jira and §Publishing rich text.
>
...
2. Convert Markdown → **ADF JSON** with the bundled converter. Do NOT hand-author wiki markup — the `acli` skill's canonical, validated path is ADF:
   ...
   ```bash
   bun .agents/skills/acli/scripts/md-to-adf.ts /tmp/qa-credentials.md /tmp/qa-credentials.adf.json
   ```
...
3. Create the Epic, then set its description from the ADF file (`acli` descriptions accept an ADF JSON file via `--description-file`):
   ...
   Exact syntax + the rich-text recipe table: `.agents/skills/acli/SKILL.md` §"Publishing rich text".
