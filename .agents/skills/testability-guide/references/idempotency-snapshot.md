...
/* qa-guide-snapshot
   ...
   page-features=shiki,request-cards,playwright-cli  // Q6/Q7/Q8 page-craft choices (see below)
   language=es
   publisher=jira-epic
...
| `page-features`      | Q6 / Q7 / Q8 (detection-pre-answered)           | Comma-separated page-craft flags (e.g. `shiki,request-cards,playwright-cli`). A missing/older flag set on a re-run signals a **flat-page upgrade opportunity** — see "page-features upgrade" below. Does NOT auto-trigger a patch; the skill OFFERS the upgrade. |
...
## `page-features` — the flat-page upgrade signal

`page-features=` records the Q6/Q7/Q8 page-craft choices baked into the current page. It is an OPTIONAL field: a page generated by an older skill version (before page-craft questions existed) will not have it at all.

Recognized flags (extend as the skill adds page-craft capabilities):

| Flag             | Means                                                          | Owning question |
| ---------------- | ------------------------------------------------------------- | --------------- |
| `shiki`          | Server-side syntax highlighting (or host-equivalent) is wired. | Q6              |
| `plain-pre`      | Code blocks are plain `<pre>` — no highlighter.                | Q6              |
| `request-cards`  | §5 auth renders Postman-style RequestCards (Visual/curl toggle). | Q8           |
| `curl-blocks`    | §5 auth renders plain highlighted curl blocks.                 | Q8              |
| `playwright-cli` | §6(b) agentic driver is the `playwright-cli` binary.          | Q7              |
| `playwright-mcp` | §6(b) agentic driver is the Playwright MCP.                    | Q7              |

**Upgrade detection on re-run**:

- **No `page-features` field present** (older flat page) → the page predates page-craft. The skill REPORTS the upgrade opportunity: _"This `/qa` predates server-highlight + request-cards + playwright-cli. Want me to upgrade it? (regenerates §5 + §6 + adds the highlight pipeline; preserves your detected config + hand-edits)"_. Do NOT auto-apply — offer it. If the user declines, write the page's CURRENT flags into the field so the next run does not re-prompt.
- **`page-features` present but lower-fidelity than detection's default** (e.g. snapshot says `plain-pre,curl-blocks,playwright-mcp` but Q6/Q7/Q8 detection would now default to `shiki,request-cards,playwright-cli`) → same OFFER, scoped to only the differing flags.
- **`page-features` matches detection** → no page-craft drift; treat as no-op for these flags.

A `page-features` change is **opt-in upgrade**, never a forced patch — it changes how the page looks, not whether it is correct. Respect the user's choice and persist it.
...
| `page-features` (upgrade, opt-in)  | the differing flags only: `shiki` → add the `_lib/highlight` + `_lib/prepare` pipeline + re-emit code blocks; `request-cards` → §5 auth from curl-blocks to RequestCards; `playwright-cli` → §6(b) + ArchDiagram UI badge from MCP to CLI. Applied ONLY on user opt-in. |
