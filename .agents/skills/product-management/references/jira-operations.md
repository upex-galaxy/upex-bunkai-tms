...
| ----------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Primary     | `/acli` skill        | `.agents/skills/acli/SKILL.md` — local CLI, lowest token cost, best for scripting and bulk reads.                                                    |
...
| Update existing ADF custom field       | **`/acli` cannot edit custom fields on existing issues** — escalate immediately | Atlassian MCP, one call per field (see `jira-publishing-gotchas.md` Gotcha 2) | REST `PUT /rest/api/3/issue/{KEY}` per field — canonical workaround documented in `.agents/skills/acli/SKILL.md` §Publishing rich text |
